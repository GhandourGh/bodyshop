...
import {
  ...
  AlertTriangle, Globe, RotateCcw, Copy, Check,
} from 'lucide-react'
...
export default function AIJob() {
  ...
  const [info, setInfo]           = useState<JobInfo>({ customerName: '', vehicleMake: '', vehicleModel: '', vehicleYear: new Date().getFullYear().toString(), language: 'en' })
  ...
  function confirmJob() {
    setStep('done')
    toast('Job confirmed! All AI predictions saved.', 'success')
  }

  function reset() {
    setStep('info'); setImageFile(null); setImageURL(''); setResults(null)
    setEditable(null); setActiveStep(-1)
    setInfo({ customerName: '', vehicleMake: '', vehicleModel: '', vehicleYear: new Date().getFullYear().toString(), language: 'en' })
  }
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input value={info.customerName} onChange={e => setInfo(p => ({ ...p, customerName: e.target.value }))}
                    ...
                    className="w-full px-4 py-3 glass border border-forge-border rounded-xl text-sm text-forge-text placeholder:text-forge-muted/40 focus:outline-none focus:border-forge-blue/50 bg-transparent" />
                </div>
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <div
                ...
                className={`relative rounded-2xl border-2 border-dashed cursor-pointer transition-all overflow-hidden ${
                  dragging ? 'border-forge-blue bg-forge-blue/5' : imageURL ? 'border-forge-blue/50' : 'border-forge-border hover:border-forge-blue/40'
                }`}
                style={{ minHeight: 260 }}>
          ...
          {step === 'analyzing' && (
            <motion.div key="analyzing" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <div className="text-center">
                ...
                <p className="text-forge-muted text-sm mt-1">Running 4 models simultaneously</p>
              </div>
            ...
            <motion.div key="review" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <div className="glass rounded-2xl p-6">
                ...
                {results.detections.length === 0 ? (
                  <p className="text-forge-muted text-sm">No specific damage classes detected. Review image manually.</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {results.detections.map((d, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-1.5 glass rounded-lg border border-forge-border/40 text-sm">
                        <span className="text-forge-text capitalize">{d.class}</span>
                        <span className="text-xs font-mono text-forge-muted">{(d.confidence * 100).toFixed(0)}%</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {imageURL && (
                  <img src={imageURL} alt="Uploaded car" className="w-full mt-4 rounded-xl object-cover max-h-48" />
                )}
              </div>

              {/* Repair time & cost — editable */}
              <div className="grid grid-cols-2 gap-4">
                <div className="glass rounded-2xl p-6">
                  ...
                  <p className="text-xs text-forge-muted text-center mt-1 font-mono">hours</p>
                </div>
                ...
                <div className="glass rounded-2xl p-6">
                  ...
                  <p className="text-xs text-forge-muted text-center mt-1 font-mono">USD</p>
                </div>
              </div>
                ...
                {results.topMechanic ? (
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs font-mono text-forge-muted">
                        <span>Skill: {results.topMechanic.skill_level}/5</span>
                        <span>Workload: {results.topMechanic.workload}%</span>
                        <span>Match: {(results.topMechanic.score * 100).toFixed(0)}%</span>
                      </div>
                    </div>
                  ...
                  <input value={editable.mechanic} placeholder="Mechanic name"
                    onChange={e => setEditable(p => p ? { ...p, mechanic: e.target.value } : p)}
                    className="w-full px-3 py-2 glass border border-forge-border rounded-xl text-sm text-forge-text placeholder:text-forge-muted/40 focus:outline-none focus:border-forge-blue/50 bg-transparent" />
                )}
              </div>

              {/* Customer message — editable */}
              <div className="glass rounded-2xl p-6">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={confirmJob}
                  className="flex-1 py-3.5 bg-forge-orange hover:bg-forge-orange-light text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
                  <CheckCircle size={16} /> Confirm Job
                </button>
              </div>
            </motion.div>
          )}
          ...
          {step === 'done' && editable && results && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div>
                <h2 className="font-display font-bold text-2xl mb-2">Job Confirmed</h2>
                <p className="text-forge-muted text-sm">AI analysis complete for {info.customerName}</p>
              </div>
